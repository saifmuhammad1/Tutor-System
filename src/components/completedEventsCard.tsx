...
import { UpcommingSessionProps } from "./upcommingEventsCard";
        ...
        {result.map((obj: ISession, indexObj: number) => (
          <div className="grid grid-cols-12 text-xs md:text-sm md:px-2  text-gray-500  transition duration-1000 hover:border-r-2 ">
            ...
            <div className="flex flex-col justify-start col-span-8  ">
              ...
              <div className="flex flex-row gap-5 text-gray-700  items-center  justify-between  md:justify-start text-xs">
                <p className="bg-primaryBlue w-fit text-white p-1  rounded-sm text-center  block md:hidden">
                  {obj.day}
                </p>
                <p>{obj.date}</p>
